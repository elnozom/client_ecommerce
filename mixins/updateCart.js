...
export default {
  ...
  methods: {
    ...
    loginRequired(productProp, qtyProp , whole = false) {
      const snackbar = {
        active: true,
        text: 'login_reuired'
      }
      ...
      if (!this.product.hasOptions) {
        ...
        if(whole)  localStorage.setItem('whole', 1)
      }
    ...
    addToCart() {
      ...
      const whole = typeof this.$route.query.whole == 'undefined' ? this.product.whole : this.$route.query.whole
      const payload = { product: this.product.id, qty: this.qty , whole  }
      this.plainAddToCart(payload)
