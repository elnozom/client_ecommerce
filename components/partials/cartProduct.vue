        ...
        <v-col cols="12">
            <div class="d-flex space-between">
                <div class="d-flex right">
                    <div class="img pointer" v-if="product.image" @click.prevent="$router.push({name : `shop-id___${$i18n.locale}` , params : {id : product.id}})">
                        <v-img  :src="product.image" lazy-src="https://qomra.pro/wp-content/uploads/2019/03/image-placeholder-350x350.png"/>
                    </div>
                    <div  v-else class="img pointer"  @click.prevent="$router.push({name : `shop-id___${$i18n.locale}` , params : {id : product.id}})">
                    ...
                    <div class="data pointer" @click.prevent="$router.push({name : `shop-id___${$i18n.locale}` , params : {id : product.id}})">
                        ...
                        <div v-else>
                            <h2 class="product-title"  v-if="$i18n.locale == 'ar'">{{product.ItemNameWhole}}</h2>
                            <h2 class="product-title"  v-else>{{product.ItemNameEnWhole}}</h2>
                        </div>
                        ...
                        <span class="text-primary d-block">{{ $n(product.price, 'currency' , 'en') }} </span>
